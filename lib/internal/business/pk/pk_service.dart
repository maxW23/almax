...
class PKService implements PKServiceInterface {
  ...
  UserEntity? get localUser => ZEGOSDKManager().currentUser;
  ...
  Future<PKInviteSentResult> invitePKBattle(
    ...
    if (pkInfo != null) {
      ...
      pkExtendedMap['user_id'] = localUser?.iduser;
  ...
  Future<void> acceptPKBattle(String requestID) async {
    // Ignore if we are currently tracking a different PK request
    if (pkInfo != null && pkInfo!.requestID != requestID) {
      debugAppLogger.debug('acceptPK ignored: requestID mismatch');
      return;
    }

    final extendedData = getPKExtendedData(PKExtendedData.START_PK) ?? '';
    try {
      // Move to pending/request state so UI can reflect progress
      pkStateNotifier.value = RoomPKState.isRequestPK;

      final result = await acceptUserRequest(requestID, extendedData);
      debugAppLogger.debug('acceptPK success callID:${result.callID}');

      // Optimistic local update; ZIM will broadcast state change anyway
      if (pkInfo != null) {
        final self = getPKUser(pkInfo!, localUser?.iduser ?? '');
        if (self != null) {
          self.callUserState = ZIMCallUserState.accepted;
        }
      }
    } catch (error) {
      debugAppLogger.debug('acceptPK fail error:$error');
  ...
  Future<ZIMCallQuitSentResult> quitPKBattle(String requestID) async {
    if (isPKUser(ZEGOSDKManager().currentUser!.iduser)) {
      if (iamHost) {
      ...
      if (timeoutQuitUsers.isNotEmpty) {
        for (final timeoutQuitUser in timeoutQuitUsers) {
          final callUserInfo = ZIMCallUserInfo(
              userID: userID, state: ZIMCallUserState.quited, extendedData: '')
  ...
  Future<void> stopPlayAnotherHostStream() async {
    ...
    for (final pkUser in pkInfo!.pkUserList.value) {
      if (pkUser.userID != cohostService?.hostNotifier.value?.iduser) {
        await ZEGOSDKManager()
            .expressService
  ...
  void updatePKRoomAttributes() {
    ...
    if (cohostService?.hostNotifier.value != null) {
      pkMap['host_user_id'] = cohostService?.hostNotifier.value?.iduser ?? '';
    }
  ...
  String? getPKExtendedData(int type) {
    ...
    final data = PKExtendedData()
      ...
      ..userName = ZEGOSDKManager().currentUser!.name
      ..type = type;
  ...
  void checkIfPKEnd(String requestID, UserEntity currentUser) {
    if (pkInfo == null) {
      return;
    }
    final selfPKUser = getPKUser(pkInfo!, ZEGOSDKManager().currentUser!.iduser);
    if (selfPKUser != null) {
      if (selfPKUser.hasAccepted) {
        var hasWaitingUser = false;
        for (final pkuser in pkInfo!.pkUserList.value) {
          if (pkuser.userID != ZEGOSDKManager().currentUser!.iduser) {
            // except self
  ...
  String hostStreamID() {
    return '${ZEGOSDKManager().expressService.currentRoomID}_${ZEGOSDKManager().currentUser!.iduser}_main_host';
  }

  String anotherHostStreamID() {
    return '${pkUser?.roomID}_${pkUser?.iduser}_main_host';
  }
  ...
  void _closeControllers() {
    ...
    try {
      onPKBattleReceived.close();
    } catch (_) {}
    try {
      onPKBattleCancelStreamCtrl.close();
    } catch (_) {}
    try {
      onPKBattleRejectedStreamCtrl.close();
    } catch (_) {}
    try {
      onPKBattleAcceptedCtrl.close();
    } catch (_) {}
    try {
      incomingPKRequestTimeoutStreamCtrl.close();
    } catch (_) {}
    try {
      outgoingPKRequestAnsweredTimeoutStreamCtrl.close();
    } catch (_) {}
    try {
      onPKStartStreamCtrl.close();
    } catch (_) {}
    try {
      onPKEndStreamCtrl.close();
    } catch (_) {}
    try {
      onPKUserJoinCtrl.close();
    } catch (_) {}
    try {
      onPKBattleTimeoutCtrl.close();
    } catch (_) {}
    try {
      onPKBattleUserQuitCtrl.close();
    } catch (_) {}
    try {
      onPKBattleUserUpdateCtrl.close();
    } catch (_) {}
    try {
      onPKUserConnectingCtrl.close();
    } catch (_) {}
