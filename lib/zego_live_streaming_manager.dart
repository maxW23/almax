...
class ZegoLiveStreamingManager implements ZegoLiveStreamingManagerInterface {
  ZegoLiveStreamingManager() {
    debugAppLogger.debug('ZegoLiveStreamingManager create');
  }
  ...
  ValueNotifier<UserEntity?> get hostNotifier =>
      cohostService?.hostNotifier ?? ValueNotifier(null);

  ListNotifier<UserEntity> get coHostUserListNotifier =>
  ...
  UserEntity? get pkUser => pkService?.pkUser;
  ...
  void init() {
    if (hadInit) {
      debugAppLogger.debug('live streaming manager, init, but had init');
      return;
    }
    debugAppLogger.debug('live streaming manager, init');
    hadInit = true;
  ...
  void uninit() {
    if (!hadInit) {
      debugAppLogger.debug('live streaming manager, uninit, but not init');
      return;
    }
    debugAppLogger.debug('live streaming manager, uninit');
    hadInit = false;
