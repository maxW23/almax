...
class ZegoNormalLivePageState extends State<ZegoNormalLivePage> {
  ...
  Widget startLiveButton(bool isLiving, RoomPKState pkState) {
    if (!isLiving && widget.role == ZegoLiveStreamingRole.host) {
      return CommonButton(
          ...
          height: 40,
          onTap: startLive,
          child: const AutoSizeText('Start Live'));
    } else {
      return const SizedBox.shrink();
    }
  }
  ...
  void startLive() {
    widget.liveStreamingManager.startLive(widget.roomID).then((value) {
      if (value.errorCode != 0) {
        ...
        //     SnackBar(content: AutoSizeText('login room failed: ${value.errorCode}')));
      } else {
        expressService
            .startPublishingStream(widget.liveStreamingManager.hostStreamID());
      }
    }).catchError((error) {
      // ignore: use_build_context_synchronously
      ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: AutoSizeText('login room failed: $error}')));
    });
  }
  ...
  void onExpressRoomStateChanged(ZegoRoomStateEvent event) {
    debugAppLogger.debug('LivePage:onExpressRoomStateChanged: $event');
  ...
  void onZIMRoomStateChanged(ZIMServiceRoomStateChangedEvent event) {
    debugAppLogger.debug('LivePage:onZIMRoomStateChanged: $event');
    ...
    if ((event.event != ZIMRoomEvent.success) &&
      ...
      //   SnackBar(
      //     duration: const Duration(milliseconds: 1000),
      //     content: AutoSizeText('onZIMRoomStateChanged: $event'),
      //   ),
      // );
    }
  ...
  void onZIMConnectionStateChanged(
    ...
    debugAppLogger.debug('LivePage:onZIMConnectionStateChanged: $event');

    // SnackbarHelper.showMessage(
    //   SnackBar(
    //     duration: const Duration(milliseconds: 1000),
    //     content: AutoSizeText('onZIMConnectionStateChanged: $event'),
    //   ),
    // );
  ...
  void onOutgoingRoomRequestRejected(OnOutgoingRoomRequestRejectedEvent event) {
    ...
    //     content:
    //         AutoSizeText('Your request to co-host with the host has been refused.'),
    //   ),
    // );
  }
  ...
  void refuseApplyCohost(RoomRequest roomRequest) {
    zimService
      ...
      // ignore: use_build_context_synchronously
      // SnackbarHelper.showMessage(
      //     SnackBar(content: AutoSizeText('Disagree cohost failed: $error')));
    });
  }
}
...
extension ZegoLivePageStateCommand on ZegoNormalLivePageState {
  ...
  void onRegisterEventCommand() {
    debugAppLogger.debug('xxxx onRegisterEventCommand');
  ...
  void onUnregisterEventCommand() {
    debugAppLogger.debug('xxxx onUnregisterEventCommand');
  ...
  void onJoinRoomCommand() {
    if (widget.role == ZegoLiveStreamingRole.audience) {
      ...
      ZEGOSDKManager()
        ...
        (value) {
          if (value.errorCode != 0) {
            debugAppLogger.debug('login room failed: ${value.errorCode}');
          }
        },
      );
      ...
      // LiveKit: enable microphone via LiveKit, do not enable Zego mic
      try {
...
class ZegoLivePageSwipingData {
  ...
  var hostNotifier = ValueNotifier<UserEntity?>(null);
}
