...
extension ZegoCallManagerExtension on ZegoCallManager {
          ...
          for (final callUser in currentCallData!.callUserList) {
            if (callUser.userID == user.userID) {
              final sdkUser = ZEGOSDKManager().getUser(user.userID)
                ...
                ..name =
                    ZEGOSDKManager().zimService.getUserName(callUser.userID) ??
                        '';
  ...
  void onInComingUserRequestReceived(IncomingUserRequestReceivedEvent event) {
    ...
    if (inRoom ||
        (currentCallData != null &&
            currentCallData?.callID != event.requestID)) {
      incomingCallInvitationReceivedStreamCtrl.add(
          IncomingCallInvitationReceivedEvent(
              callID: event.requestID, info: info));
      return;
    }
    ...
    ZEGOSDKManager().zimService.queryUsersInfo(userIDList).then((value) {
      ...
      for (final userInfo in event.info.callUserList) {
        final callUser = CallUserInfo(userID: userInfo.userID)
          ..sdkUserNoti.value = userInfo.userID == localUser?.iduser
              ? localUser
              : ZEGOSDKManager().getUser(userInfo.userID)
  ...
  void onUserRequestStateChanged(UserRequestStateChangeEvent event) {
    if (currentCallData?.callID == event.requestID) {
      debugAppLogger.debug('onUserRequestStateChanged:$event');
      for (final userInfo in event.info.callUserList) {
        ...
        if (hasUserStateUpdate) {
          onCallUserUpdateStreamCtrl.add(OnCallUserUpdateEvent(
      ...
      for (final userInfo in event.info.callUserList) {
        switch (userInfo.state) {
          case ZIMCallUserState.received:
            debugAppLogger
                .debug('onUserRequestStateChanged received:${userInfo.userID}');
            receivedUsers.add(userInfo);
            onReceiveCallUserReceived(receivedUsers);
            break;
          case ZIMCallUserState.accepted:
            debugAppLogger
                .debug('onUserRequestStateChanged accept:${userInfo.userID}');
            onReceiveCallUserAccepted(userInfo);
            break;
          case ZIMCallUserState.rejected:
            debugAppLogger
                .debug('onUserRequestStateChanged reject:${userInfo.userID}');
            outgoingCallInvitationRejectedStreamCtrl.add(
          ...
          case ZIMCallUserState.timeout:
            outgoingCallInvitationTimeoutStreamCtrl.add(
                OutgoingCallTimeoutEvent(
                    userID: userInfo.userID,
                    extendedData: userInfo.extendedData));
            if (localUser != null) {
              checkIfPKEnd(event.requestID, localUser!);
  ...
  void onReceiveCallUserReceived(List<ZIMCallUserInfo> userInfoList) {
    if (userInfoList.isNotEmpty) {
      ...
      ZEGOSDKManager().zimService.queryUsersInfo(userIDList).then((value) {
        onCallUserInfoUpdateStreamCtrl
            .add(OnCallUserInfoUpdateEvent(userList: userIDList));
      });
    }
  }
  ...
  void onReceiveCallUserAccepted(ZIMCallUserInfo userInfo) {
    ...
    for (final callUser in currentCallData!.callUserList) {
      if (callUser.userID == localUser?.iduser) {
    ...
    onOutgoingCallInvitationAccepted.add(OnOutgoingCallAcceptedEvent(
        userID: userInfo.userID, extendedData: userInfo.extendedData));
  }
  ...
  void onReceiveCallUserQuit(String requestID, ZIMCallUserInfo userInfo) {
    if (currentCallData != null) {
      final selfCallUser =
          getCallUser(currentCallData!, localUser?.iduser ?? '');
      if (selfCallUser != null && selfCallUser.hasAccepted.value) {
        ...
        if (moreThanOneAcceptedExceptMe) {
          onCallUserQuitStreamCtrl.add(CallUserQuitEvent(
              userID: userInfo.userID, extendedData: userInfo.extendedData));
        }
        if (!hasWaitingUser) {
          quitCall();
  ...
  void checkIfPKEnd(String requestID, UserEntity currentUser) {
    ...
    final selfCallUser = getCallUser(currentCallData!, currentUser.iduser);
